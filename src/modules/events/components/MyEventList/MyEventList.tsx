...
import styles from "../../../additional/components/MyAdditionalList/style.module.css";

type Props = {
  items: EventItemType[]
};

export function MyEventList({items}: Props) {
      ...
      {items.map((item) => (
        <li className={styles.card} key={item.id}>
          <div className={styles.dateContainer}>
            ...
            <div className={styles.date}>{item.end_date} • {item.end_time}</div>
          </div>
